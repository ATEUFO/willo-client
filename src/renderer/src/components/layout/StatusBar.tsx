...
export const StatusBar: React.FC = () => {
  const { currentRole, isOnline, hospitalSettings, pendingCacheSync, setShowConnectionsModal } = useHospitalStore()
      ...
      <footer className="h-7 bg-[#050D1A] border-t border-slate-800 text-slate-300 text-[11px] font-mono flex items-center justify-between px-3 select-none z-50 shrink-0 shadow-inner">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isOnline ? (
              <>
                ...
                <span className="font-semibold text-emerald-400">Connecté au Serveur</span>
              </>
            ) : (
              <>
                <WifiOff className="w-3 h-3 text-amber-400" />
                <span className="font-semibold text-amber-300">Base de Données Locale</span>
              </>
            )}
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-slate-400 hover:text-white hover:bg-slate-800/80 px-2 py-0.5 rounded transition-colors cursor-pointer"
            title={`Cliquer pour voir le journal des données non synchronisées (${pendingCacheSync} en attente)`}
          >
