...
export const LoginPage: React.FC<LoginPageProps> = ({ onSwitchToSignin, onLoginSuccess }) => {
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        if (!username.trim()) {
            setErrorMessage("Veuillez saisir votre nom d'utilisateur ou identifiant.")
            return
        }
    ...
    const quickLogin = (role: Role, demoUser: string) => {
            ...
            <div className="w-full max-w-md space-y-6">
                ...
                <div className="text-center space-y-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium">
                            Système d'Information Hospitalier (SIH) • Connexion Poste
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-medical-cardBg border border-medical-border rounded-2xl p-6 sm:p-8 shadow-xl space-y-5">
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-medical-border rounded-xl text-xs text-slate-900 focus:outline-none focus:border-medical-primary focus:bg-white transition-all font-semibold"
                                >
                    ...
                    <div className="border-t border-medical-border pt-4 space-y-2">
                        <span className="text-[11px] font-semibold text-slate-500 block">Démos Rapides :</span>
                        <div className="grid grid-cols-3 gap-2">
                            <button
                                type="button"
                                onClick={() => quickLogin('consultation', 'dr.kouassi')}
                            ...
                            <button
                                type="button"
                                onClick={() => quickLogin('reception', 'accueil.triage')}
                                className="p-2 bg-slate-50 hover:bg-medical-subtle border border-medical-border rounded-xl text-[11px] font-semibold text-slate-700 flex flex-col items-center gap-1 transition-colors"
                            >
                            ...
                            <button
                                type="button"
                                onClick={() => quickLogin('admin', 'admin.sys')}
                                className="p-2 bg-slate-50 hover:bg-medical-subtle border border-medical-border rounded-xl text-[11px] font-semibold text-slate-700 flex flex-col items-center gap-1 transition-colors"
                            >
                ...
                <p className="text-center text-[11px] text-slate-400 font-mono">
                    WILLO Client v1.0.0 • Solution 100% Offline / Local Network
                </p>
            </div>
        </div>
    )
